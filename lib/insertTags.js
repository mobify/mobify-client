module.exports = function insertTags(html, o) {    
    var scriptRe = /(<script[\s\S]*?<\/script[\s\S]*?>)/gmi
      ...
      , closeTextareaRe = /(<\/textarea>)/gmi
        // Mobify.snippet
      , mobifyRe = /Mobify\./
      , mobifyTagRe = /<!-- MOBIFY - DO NOT ALTER -->[\s\S]*?<!-- END MOBIFY -->/gmi
      , opts = '{path:"' + o.siteFolderPath + '"}';

    // Remove Mobify <script> and escape <style> inside <script>.
    ...
    html = html.replace(openHeadRe, function(str) {
        ...
        if (o.mobifyJsPath) {
            tag = tag.replace('http://localhost:8080/mobify.js', o.mobifyJsPath);
        }
    ...
    if (!hasOpenHead) throw new Error('No open head.')
