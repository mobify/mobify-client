...
var maybeCompress = function(text) {
    var shouldUglify = dust.uglify;
...
dust.ark = function(data, chunk, params) {    
    ...
    if (params.passive) {
        result = maybeCompress(data);
        body = '"' + dust.escapeJs(result[1]) + '",';
    } else {
        body = 'function(){' + data + '},'
    }
    var err = result[0] && ('(console&&console.warn("' + dust.escapeJs(result[0]) + '"));\n');
    return chunk.write(err + 'Mobify.ark.store(').write(name).write(body).write(skipExecution)
        .write(');').end();
}
...
function makePragmaHandler(key, source) {
    return function(compiler, context, bodies, params) {
        ...
        if (source) {
            ...
            // Pragma parsing rules are restrictive, preventing us from passing parameters
            // with slashes and other useful characters. To pass such parameters anyway,
            // we use {@val}{/val} helper as first child inside pragma. The code below
            // will find the value placed in such a helper.
            // The helpers will be injected by pragmify() function above.
            // And yes, whut.
